...
export default class TodoApp extends Component {
    ...
    constructor(props) {
        super(props);
        const listaInicial = [];    
        this.state = {items: listaInicial, description:"", name: '', email: '', status: '', dueDate: null, file: null,
    ...
    handleFilter(e) {    
        ...
        for (var i = 0; i < this.state.items.length; i++) {
            ...
            if (item.dueDate.toString() === this.state.dueFilter || 
                item.responsible.email === this.state.emailFilter || 
    ...
    showEmails() {
        ...
        for (var i = 0; i < this.state.items.length; i++) {
            const item = this.state.items[i];
            setEmais.add(item.responsible.email);
        }
        ...
        axios.post('http://localhost:8080/api/todo', 
            task
             ...
             .catch(e => {    
                console.log("Error");
                console.log(e);
                alert("An error has occurred try add TODO!");
             });        
    }
    refresAll = (newItem) => {
        this.setState({
